...
// NODMA is a debug mode that disables the DMA engine and prints raw PIO FIFO
// outputs it is limited to testing a small number of samples equal to the PIO
// FIFO depths #define NODMA 1 These two enable debug print outs of D4
// generation, D4_DBG2 are consider higher verbosity #define D4_DBG 1 #define
// D4_DBG2 2
...
volatile int
    lowerhalf; // are we processing the upper or lower half of the data buffers
...
// CR/LF in certain modes. The tud_cdc_write_available function returns 256,
// and thus we have a 256B buffer to feed into but the CDC serial issues in
// groups of 64B. Since there is another memory fifo inside the TUD code this
// might possibly be optimized to directly write to it, rather than writing
// txbuf. That might allow faster rle processing but is a bit too complicated.
...
// send counts of repeated values to effeciently utilize USB CDC link
// bandwidth. This is the only mode where a given serial byte can have both
// sample information and RLE counts. Samples from PIO are dma'd in 32 bit
// words, each containing 8 samples of 4 bits (1 nibble). RLE Encoding: Values
// 0x80-0xFF encode an rle cnt of a previous value with a new value:
...
uint32_t send_slices_D4(sr_device_t *d, uint8_t *dbuf) {
  ...
  // make the for loop easier, and setup the initial conditions for rle
  // tracking
  ...
  // If in fixed sample (non-continous mode) only send the amount of samples
  // requested
  ...
  for (int i = 0; i < (samp_remain >> 3); i++) {
    ...
    // accumulate a few of them push to the device so that we don't
    // accumulate large numbers of unsent RLEs.  That allows the host to
    // process them gradually rather than in a flood when we get a value
    // change.
    while (rlecnt >= 640) {
      txbuf[txbufidx++] = 127;
      rlecnt -= 640;
    ...
    // cases with low activity factors We must make sure cword==lword and
    // that all nibbles of cword are the same
    if ((cword == lword) && ((cword >> 4) == (cword & 0x0FFFFFFF))) {
      ...
      for (int j = 0; j < 8; j++) { // process all 8 nibbles
        ...
        if (nibcurr == niblast) {
          ...
          // If the value changes we must push all remaing rles to the
          // txbuf chngcnt++; Send intermediate 8..632 RLEs
          if (rlecnt > 7) {
...
uint32_t get_cval(uint8_t *dbuf) {
  ...
  if (d_dma_bps == 1) {
    ...
    // To make a 32bit value written from PIO we pull in IOs that aren't
    // actuall digital channels so mask them off
    cval <<= 11;
    cval >>= 11;
  }
  rxbufdidx += d_dma_bps;
...
// respectively. We can just always read a 4B value because the core doesn't
...
// 4B is 17-21 channels and is the only one that must mask invalid bits which
// are captured by DMA
...
// All digital channels for one slice are sent first in 7 bit bytes using
// values 0x80 to 0xFF Analog channels are sent next, with each channel taking
// one 7 bit byte using values 0x80 to 0xFF. This does not support run length
// encoding because it's not clear how to define RLE on analog signals
...
// See if a given half's dma engines are idle and if so process the data,
// update the write pointer and ensure that when done the other dma is still
// busy indicating we didn't lose data . int
// __not_in_flash_func(check_half)(sr_device_t *d,volatile uint32_t
// *tstsa0,volatile uint32_t *tstsa1,volatile uint32_t *tstsd0,
int check_half(sr_device_t *d, volatile uint32_t *tstsa0,
               volatile uint32_t *tstsa1, volatile uint32_t *tstsd0,
               volatile uint32_t *tstsd1, volatile uint32_t *t_addra0,
               volatile uint32_t *t_addrd0, uint8_t *d_start_addr,
               uint8_t *a_start_addr, bool mask_xfer_err) {
  int a0busy, d0busy;
  ...
  if (((a0busy == 0) || (d->a_mask == 0)) &&
    ...
    // When one half finishes we send it's data while the other dma
    // controller writes to the other half. We rely on DMA chaining where one
    // completing dma engine triggers the next. When chaining happens the
    // original target address is not restored so we must rewrite the
    // starting address. Also when chaining is enabled we can get into an
    // infinite loop where we ping pong between each other. and it is
    // possible that if the other DMA controller finishes before we have send
    // our half of the data that our DMA controller will startup and
    // overwrite our data.  That is the DMA overflow condition. We don't
    // actually overflow the DMA, but instead we fail to establish proper
    // chaining to keep the ping pong going. The only way to avoid the
    // overflow condition is to reduce the sampling rate so that the transmit
    // of samples can keep up, or do a fixed sample that fits into the sample
    // buffer. When we first enter the loop it is assumed that the other
    // controller is not chained to us so that if we don't process in time
    // then our DMA controller won't be started by the other.  That
    // assumption is maintained by always pointing our DMA engine to itself
    // so tht in the next function call it is the "other" controller. We then
    // process our half's samples and send to USB. When done we confirm that
    // the other channel is still active, and if so we know we haven't
    // overflowed and thus can establish the chain from the other channel to
    // us.  If the other channel is not still active, or if the PIO/ADC FIFOs
    // indicates an RXstall condition which indicates PIO lost samples, then
    // we abort. Note that in all cases we should never actually send any
    // corrupted data we just send less than what was requested. Note that we
    // must use the "alias" versions of the DMA CSRs to prevent writes from
    // triggering them. Since we swap the csr pointers we determine the other
    // half from the address offsets.
    ...
    // Ensure other dma is still busy, if not that means we have samples from
    // PIO/ADC that could be lost. It's only an error if we haven't already
    // gotten the samples we need, or if we are processing the first half and
    // all the remaining samples we need are in the 2nd half. Note that in
    // continuous mode num_samples isn't defined.
    uint8_t proc_fail;
    ...
    //		  &&((((*tstsd1)>>24)&1)||(d->d_mask==0)))){
    if (mask_xfer_err || ((piorxstall1 == 0) && (adcfail == 0) &&
                          (piorxstall2 == 0) && (proc_fail == 0))) {
      //           Dprintf("h\n\r");
      return 1;
    } else {
      if (piorxstall1 || piorxstall2) {
        Dprintf("***Abort PIO RXSTALL*** %d %d half %d\n\r", piorxstall1,
                piorxstall2, num_halves);
      }
      if (proc_fail) {
      ...
      // The main loop also sends these periodically until the host is
      // done..
...
// Check if dma activity is complete.  This was split out to allow core1 to do
// the monitoring and slice processing and leave usb interrupt handling to core
// 0, but doing so did not improve streaming performance, so it's left in
// core0.
  ...
  if (d->sending && d->started && ((d->scnt < d->num_samples) || d->cont)) {
    ...
    if (lowerhalf) {
      ret = check_half(&dev, tstsa0, tstsa1, tstsd0, tstsd1, taddra0, taddrd0,
                       &(capture_buf[d->dbuf0_start]),
                       &(capture_buf[d->abuf0_start]), mask_xfer_err);
    ...
    if (lowerhalf == 0) {
      ret = check_half(&dev, tstsa1, tstsa0, tstsd1, tstsd0, taddra1, taddrd1,
                       &(capture_buf[d->dbuf1_start]),
                       &(capture_buf[d->abuf1_start]), mask_xfer_err);
      if (ret == 1) {
        lowerhalf = 1;
      } else if (ret < 0) {
        d->sending = false;
...
// This is a simple maintenance loop to monitor serial activity so that core0
// can be dedicated to monitoring DMA activity and sending trace data. Most of
// the time this loop is stalled with wfes (wait for events).
void core1_code() {
  ...
  while (true) {
    ...
    // Each core instruction takes a memory cycle, as does each core memory
    // or IO register read. The memory fabric supports up to 4 reads per
    // cycle if there are no bank conflicts The DMA engine needs a read of
    // the PIO and ADC and a write of memory and thus chances of conflicts
    // are high.  Thus once we are in sampling mode slow down C1 to allow C0
    // to loop faster and process faster. Without wfes C1 typically completes
    // 10x the number of outer loops as C0, but with the wfes C0 completes
    // more loops and C1 activity drops to 1% . Much of the C0 code has built
    // in sev (Send Event) but we also add an explicit one in the main while
    // loop
    ...
    // The '+' is the only character we track during normal sampling because
    // it can end a continuous trace.  A reset '*' should only be seen after
    // we have completed normally or hit an error condition.
    if (intin == '+') {
      dev.sending = false;
      dev.aborted = false; // clear the abort so we stop sending !!
    }
    // send_resp is used to eliminate all prints from core1 to prevent
    // collisions with prints in core0
    else if (intin >= 0) {
...
int main() {
  ...
  sleep_us(100000);
  // GPIOs 26 through 28 (the ADC ports) are on the PICO, GPIO29 is not a pin
  // on the PICO
  adc_gpio_init(26);
  adc_gpio_init(27);
  adc_gpio_init(28);
  ...
  tcounta0 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan0 +
                                   0x8); // DMA_TRANS_COUNT offset
  tcounta1 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan1 +
                                   0x8); // DMA_TRANS_COUNT offset
  tcountd0 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan0 +
                                   0x8); // DMA_TRANS_COUNT offset
  tcountd1 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan1 +
                                   0x8); // DMA_TRANS_COUNT offset

  // Use the debug version of the count registers because the count of the
  // DMA_TRANS_COUNT is not visible if it is not active
  volatile uint32_t *tcountdbga0, *tcountdbga1, *tcountdbgd0, *tcountdbgd1;
  tcountdbga0 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan0 +
                                      0x804); // DMA_TRANS_COUNT DBGoffset
  tcountdbga1 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan1 +
                                      0x804); // DMA_TRANS_COUNT DBGoffset
  tcountdbgd0 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan0 +
                                      0x804); // DMA_TRANS_COUNT DBGoffset
  tcountdbgd1 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan1 +
                                      0x804); // DMA_TRANS_COUNT DBGoffset

  taddra0 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan0 +
                                  0x4); // DMA_WRITE_addr offset
  taddra1 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan1 +
                                  0x4); // DMA_WRITE_addr offset
  taddrd0 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan0 +
                                  0x4); // DMA_WRITE_addr offset
  taddrd1 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan1 +
                                  0x4); // DMA_WRITE_addr offset

  tstsa0 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan0 +
                                 0xc); // DMA_WRITE_sts offset
  tstsa1 = (volatile uint32_t *)(DMA_BASE + 0x40 * admachan1 +
                                 0xc); // DMA_WRITE_sts offset
  tstsd0 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan0 +
                                 0xc); // DMA_WRITE_sts offset
  tstsd1 = (volatile uint32_t *)(DMA_BASE + 0x40 * pdmachan1 +
                                 0xc); // DMA_WRITE_sts offset
  // PIO status
  ...
  */
  ...
  while (1) {
    ...
    if (dev.sending && (dev.started == false)) {
      // Only boost frequency during a sample so that average device power
      // is less. It's not clear that this is needed because rp2040 is
      // pretty low power, but it can't hurt...
#ifdef SYS_CLK_BOOST_EN
      // This is not enabled if any analog channels are enabled because due
      // to ADC frequency limits the processing rate is never the limiter
      // to performance. And maybe it might help avoid an overfrequency
      // issue with ADC...or not...
      if (dev.a_chan_cnt == 0) {
        Dprintf("Boost up\n\r");
      ...
      // because a frequency step of 2 is required to get a pulldown to
      // specify the sample rate, but sigrok cli can still pass it.
      dev.sample_rate >>= 1;
      dev.sample_rate <<= 1;
      ...
      // Calculate relative size in terms of nibbles which is the smallest
      // unit, thus a_chan_cnt is multiplied by 2 Nibble size storage is
      // only allow for D4 mode with no analog channels enabled For
      // instance a D0..D5 with A0 would give 1/2 the storage to digital
      // and 1/2 to analog
      ...
      // multiple of 32  because the dma buffer is split in half, and the
      // PIO does writes on 4B boundaries, and then a 4x factor for any
      // other size/alignment issues
      ...
      Dprintf("chunk size %d samples %d buff %d needed %d\n\r", chunk_size,
              chunk_samples, buff_chunks, chunks_needed);
      Dprintf("dbytes per chunk %d dig samples per chunk %d\n\r",
              dig_bytes_per_chunk, dig_samples_per_chunk);
      // If all of the samples we need fit in two half buffers or less then
      // we can mask the error logic that is looking for cases where we
      // didn't send one half buffer to the host before the 2nd buffer
      // ended because we only use each half buffer once.
      mask_xfer_err = false;
      // If requested samples are smaller than the buffer, reduce the size
      // so that the transfer completes sooner. Also, mask the sending of
      // aborts if the requested number of samples fit into RAM Don't do
      // this in continuous mode as the final size is unknown
      if (dev.cont == false) {
      ...
      // Dprintf("Final sizes d %d a %d mask err %d samples per half
      // %d\n\r"
      ...
      *adcfcs |= 0xC00;
      ...
      // Enable the initial chaing from the first half to 2nd, further
      // chains are enabled based on whether we can parse each half in
      // time.
      ...
      // Dprintf("start offsets d0 0x%X d1 0x%X a0 0x%X a1 0x%X samperhalf
      // %u\n\r"
      ...
      if (dev.a_chan_cnt) {
        adc_run(false);
        //             en, dreq_en,dreq_thresh,err_in_fifo,byte_shift to
        //             8 bit
        adc_fifo_setup(false, true, 1, false, true);
        adc_fifo_drain();
        // Dprintf("astart cnt %u div
        // %f\n\r",dev.a_chan_cnt,(float)adcdivint);

        // This sdk function doesn't support support the fractional
        // divisor
        ...
        // For the case of a requested 500khz clock, we would normally
        // write a divisor of 95, but doesn't give the desired result, so
        // we use the 0 value instead. Fractional divisors should
        // generally be avoided because it creates skew with digital
        // samples.
        ...
        // This is needed to clear the AINSEL so that when the round
        // robin arbiter starts we start sampling on channel 0
        adc_select_input(0);
        ...
        // set chan0 to immediate trigger (but without adc_run it
        // shouldn't start), chan1 is chained to it.
      ...
      if (dev.d_mask) {
        ...
        // Due to how PIO shifts in bits, if any digital channel within a
        // group of 8 is set, then all groups below it must also be set.
        // We further restrict it in the tx_init function by saying
        // digital channel usage must be continous.
        /* pin count is restricted to 4,8,16 or 32, and pin count of 4 is
       only used Pin count is kept to a powers of 2 so that we always
       read a sample with a single byte/word/dword read for faster
       parsing. if analog is disabled and we are in D4 mode bits
       d_dma_bps   d_tx_bps 0-4    0 1 No analog channels 0-4    1 1 1 or
       more analog channels 5-7 1          1 8      1          2 9-12   2
       2 13-14  2          2 15-16  2          3 17-21  4          3
       */
        dev.pin_count = 0;
        ...
        if (dev.d_mask & 0x0FFF0000)
          dev.pin_count += 16;
        // If 4 or less channels are enabled but ADC is also enabled, set
        // a minimum size of 1B of PIO storage
        ...
        // Configure state machine to loop over this `in` instruction
        // forever, with autopush enabled.
        ...
        // Unlike the ADC, the PIO int divisor does not have to
        // subtract 1. Frequency=sysclkfreq/(CLKDIV_INT+CLKDIV_FRAC/256)
        sm_config_set_clkdiv_int_frac(&c, div_int, frac_int);

        // Since we enable digital channels in groups of 4, we always get
        // 32 bit words
        ...
        //                       number    config   buffer target piosm
        //                       xfer size  trigger
        ...
        // This is done later so that we start everything as close in
        // time as possible
        //              pio_sm_set_enabled(pio, piosm, true);
      } // dev.d_mask
      // These must be at their initial value,(or zero for the 2ndhalf)
      // otherwise it indicates they have started to countdown
      // Dprintf("Tcount start d %u %u a %u
      // %u\n\r",*tcountd0,*tcountd1,*tcounta0,*tcounta1); These must be
      // the initial value for both Dprintf("Tcount dbg start d 0x%X 0x%X a
      // 0x%X
      // 0x%X\n\r",*tcountdbgd0,*tcountdbgd1,*tcountdbga0,*tcountdbga1);
      // These catch cases in DMA coding where DMA engines have started too
      // soon..
      ...
      // Dprintf("dma addr start d 0x%X 0x%X a 0x%X
      ...
      // 0x%X\n\r",*pioctrl,*piofstts,*piodbg,*pioflvl); Dprintf("DMA
      // channel assignments a %d %d d %d
      // %d\n\r",admachan0,admachan1,pdmachan0,pdmachan1); Dprintf("DMA ctr
      // reg addrs a %p %p d %p %p\n\r",(void *) tstsa0,(void
      // *)tstsa1,(void
      // *)tstsd0,(void *)tstsd1); Dprintf("DMA ctrl reg a 0x%X 0x%X d 0x%X
      // 0x%X\n\r",*tstsa0,*tstsa1,*tstsd0,*tstsd1); Enable logic and
      // analog close together for best possible alignment warning - do not
      // put printfs or similar things here...
      tstart = time_us_32();
    ...
    // In high verbosity modes the host can miss the "!" so send these until
    // it sends a "+"
    if (dev.aborted == true) {
    ...
    if ((dev.sending == false) && (init_done == true)) {
      // Dprintf("Ending PIO ctrl 0x%X fstts 0x%X dbg 0x%X lvl
      // 0x%X\n\r",*pioctrl,*piofstts,*piodbg,*pioflvl); The end of
      // sequence byte_cnt uses a "$<byte_cnt>+" format. Send the byte_cnt
      // to ensure no bytes were lost
      if (dev.aborted == false) {
        char brsp[16];
        // Give the host time to finish processing samples so that the
        // bytecnt isn't dropped on the wire
        sleep_us(100000);
      ...
      // Print USB Endpoint controls in the DPSRAM, which is at the base of
      // USBCTRL 0x0 is setup packet, 0x8-0xfc - EP in/out buffer control
      // 0x100
      // - EP0 buffer 0 (in and out) 0x140 - EPO buffer 1 0x180 - data
      // buffers
      /*
      volatile uint32_t *usbctrl;
      usbctrl=(volatile uint32_t *)(USBCTRL_BASE);
      for(i=0;i<256;i+=4){ //It's a 4k space, but everything above this is
      zero Dprintf("0x%03X %8X %8X %8X %8X
      \n\r",i*4,usbctrl[i],usbctrl[i+1],usbctrl[i+2],usbctrl[i+3]);
      }
      */

      // Print out debug information after completing, rather than before
      // so that it doesn't delay the start of a capture
...
// buffer management to support sample rates that were substantially higher
// than the stream rates across USB.  Thus there wasn't a compelling reason to
// have it. It's left as an example as to how the masks could be used.
...
// sent before sending other samples capture by the DMA after the trigger
// event.
/*
//   uint32_t tlval; Trigger last val
//   tlval=d->tlval;
//   uint32_t all_mask=d->lvl0mask | d->lvl1mask| d->risemask | d->fallmask |
d->chgmask;
