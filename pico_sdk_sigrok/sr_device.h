...
// Base value of sys_clk in khz.  Must be <=125Mhz per RP2040 spec and a
// multiple of 24Mhz to support integer divisors of the PIO clock and ADC clock
#define SYS_CLK_BASE 120000
// Boosted sys_clk in khz.  Runs the part above its specifed frequency limit to
// support faster processing of digital run length encoding which in some cases
// may allow for faster streaming of digital only data.
//****************************
// Use this at your own risk
//***************************
// Frequency must be a 24Mhz multiple and less than 300Mhz to avoid known
// issues The authors PICO failed at 288Mhz, but testing with 240Mhz seemed
// reliable #define SYS_CLK_BOOST_EN 1 #define SYS_CLK_BOOST_FREQ 240000
...
typedef struct {
  ...
  uint32_t
      samples_per_half; // number of samples for one of the 4 dma target arrays
...
that lval is correct
  ...
  //  uint32_t tlval; //last digital sample value - must keep it across
multiple calls to send_slices for trigger
